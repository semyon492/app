      ...
      <template #body>
        <form v-if="type == 'login'" class="w-full" @submit.prevent="login">
          <div class="flex gap-x-2 py-4 items-center  ">
            <img src="http://res.cloudinary.com/dcwekkkez/image/upload/v1696220970/qvufhaedruigxznlvvs0.jpg" alt="userImage" class="w-10 h-10 rounded-full object-cover ">
            <div>
              <div class="text-[15px] font-semibold ">Семен</div>
              <button class="px-2 py-1 flex gap-x-0.5 items-center text-[12px] bg-[#E4E6EB] dark:bg-[#3A3B3C] rounded-lg mt-0.5 font-semibold scrollbar scrollbar-thumb-sky-200 scrollbar-track-gray-100 ">
                ...
                <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" class=" " height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                ...
                <label class="block text-sm text-gray-700 dark:text-gray-400">
                  ...
                  <select class="block w-full text-sm dark:text-gray-300 focus:outline-none focus:border-purple-400 dark:border-gray-600 dark:bg-gray-700 focus:shadow-outline-purple dark:focus:shadow-outline-gray dark:focus:border-gray-600 form-select leading-5 mt-1">
          ...
          <textarea class="input-modal style-3 bg-inherit focus:ring-0 border-0 w-full placeholder:text-[#a0a0a1] text-[22px] h-[200px] relative" placeholder="What's on your mind, Семен?">
          </textarea>
          <div v-if="form_alert">
            {{ err_info }}
          </div>
