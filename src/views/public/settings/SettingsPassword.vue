    ...
    <SettingsCard>
      <div class="bg-white dark:bg-slate-900 lg:rounded-2xl">
        ...
        <form class="flex-1 p-6" @submit.prevent="change_pass">
          <v-input v-model:modelValue="form_user.oldpassword" type="password" :label="$t('settings.current_password')" />   
          <hr class="my-6 -mx-6 dark:border-slate-800 border-t border-gray-100">          
          <v-input v-model:modelValue="form_user.password" type="password" :label="$t('settings.new_password')" />
          <v-input v-model:modelValue="form_user.repassword" type="password" :label="$t('auth.password_confirm')" />
          <div v-if="form_alert">
            {{ err_info }}
          </div>
          <div class="mb-6 last:mb-0 mt-4">
            <v-button type="submit" variant="purple">{{ $t('settings.change_password') }}</v-button>
          </div>                
        </form>
      </div>
    </SettingsCard>
